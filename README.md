...
For generated marketing plans, the first priority is always LinkedIn prospect search (`linkedin_prospect_search`): find prospect clients and score whether they are hiring, active, open, warming, or quiet. The CMO spawns a dedicated Cursor subagent (`linkedin-prospector` or `plan-task`) to complete one chosen priority in an isolated context, then applies the governed state change. Propose asks which ranked priority to execute. Autopilot advances exactly one highest-ranked action that application code classifies as easy internal work (`linkedin_prospect_search`, `research_brief`, `content_draft`, or `campaign_outline`). Funnel analysis and heartbeats remain medium; outbound publication, messaging, spend, live LinkedIn actions, and live integrations are absent from the plan action vocabulary.
